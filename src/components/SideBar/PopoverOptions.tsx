    ...
    <div className="flex flex-col">
        ...
        <div className="grid grid-cols-3 gap-2">
          {options.map((option, index) => (
            <button key={index} className={`border ${option === selected ? "bg-primary-100" : ""} border-gray-200 rounded-md px-4 py-3 text-sm font-medium text-gray-900`} onClick={() => setSelected(option as T)}>
              {option}
          </button>
          ))}
        </div>
